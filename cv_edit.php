...
if( $_POST[update_cv] == 'true' )
{
				$title_cv = $_POST[title_cv];
				$detail_cv = $_POST[detail_cv];
				$stoid = $_POST[stoid];
				$sql = pg_query("UPDATE cv set title_cv = '$title_cv' , detail_cv = '$detail_cv' where id_cv = '$stoid' ;");
				header('location:cv_detail.php?stoid='.$stoid);
}
if( $_POST[update_cv] == 'false' )
{
				...
				imagedestroy( $dst1 );
							...
							$stoid = $_POST[stoid];
							$sql = pg_query("UPDATE cv set  img_cv = '$rnd_name1'
								where id_cv = '$stoid' ;");
							header('location:cv_edit.php?stoid='.$stoid);
...
if ( isset($_POST[delete_cv]) ) {
	...
	$sql_delete = pg_query("DELETE from cv where id_cv = '$stoid';");
								...
								<div class="aside-body">
									...
									<article class="article-mini">
										<div class="inner">
											...
											<hr>
											<input type="hidden" name="stoid" value="<?php echo $result[id_cv]; ?>">
											<button type="submit" title="" name="update_cv" value="true" class="btn btn-primary btn-block"><i class="fa fa-wrench" aria-hidden="true"></i> บันทึกเรื่องราว</button>
											<button  type="submit" name="delete_cv"class="btn btn-danger btn-block"  onclick="return confirm('ยืนยันการลบเรื่องราวนี้ ? ถ้าลบแล้วจะสามารถย้อนกลับได้')"><i class="fa fa-window-close" aria-hidden="true"></i>ลบเรื่องราว</button>
											<a href="profile.php#cv" class="btn  btn-sm btn-block" ><i class="fa fa-long-arrow-left" aria-hidden="true"></i></i>กลับ</a>
						...
						<div class="col-md-9">
							...
							<article class="article main-article">
								<header>
									...
									<h2>
									<input type="text"  name="title_cv" class="form-control" value="<?php echo $result[title_cv]; ?>" name="">
									</h2>
									<ul class="details">
								...
								<div class="main">
									<p>
										<textarea name="detail_cv" class="form-control"  rows="6" ><?php echo $result[detail_cv]; ?> </textarea>
										<hr>
									</p>
									<div class="col-md-6">
										เลือกภาพใหม่
